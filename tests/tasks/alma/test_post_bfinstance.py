...
def test_NewInstancetoAlma_local_id_overlay_puts_directly(mocker: MockerFixture):
    mock_get_env_vars = Mock()
    mock_get_env_vars.side_effect = lambda x: (
        ("uri_region", "alma_api_key") if x == "penn" else (None, None)
    )
    ...
    NewInstancetoAlma(dag=Mock(dag_id="penn"), task_instance=task_instance)
...
def test_NewInstancetoAlma_local_id_not_found_skips_and_notifies(
    ...
    mock_send_email = mocker.patch(
        "ils_middleware.tasks.alma.post_bfinstance.send_local_id_not_found_email"
    )
