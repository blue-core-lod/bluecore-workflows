...
mock_messages_list = [
    {
        ...
        "Body": '{"resource":{"uri":"http://exmpl/resource/foo"},"user":{"email":"abc@yale.edu"},"group":"stanford","target":"ils"}',
    },
    {
        "MessageId": "5678ab4f-52ab-4b54-a6cd-765b9876f08a",
        "ReceiptHandle": "real+long+string2",
        "MD5OfBody": "388e4db0aacfbcbe93e6072dd7cda916",
        "Body": '{"resource":{"uri":"http://exmpl/resource/bar"},"user":{"email":"abc@yale.edu"},"group":"yale","target":"ils"}',
    },
]
...
def test_send_task_failure_notifications(
    ...
    logger_spy.assert_called_with(
        f"Error executing upstream task for https://api.development.sinopia.io/resource/8888-9999-0000-1111: err_msg_context={expected_err_context}"
    )
